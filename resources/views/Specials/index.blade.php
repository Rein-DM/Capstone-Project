...
@section('title', 'Special Education Fund')
...
@if (Auth::user()->role=='Encoder')
    <div class="d-flex align-items-center justify-content-between">
        <a href="{{ route('special.create') }}" class="btn btn-primary">Add Special Education Funds</a>
    </div>
@endif
<?php /*----------------------------------------Admin-------------------------------------------------------*/ ?>
                ...
                <tr>
                        ...
                        <td class="align-middle">
                            <div class="btn-group" role="group" aria-label="Basic example">
                                <a href="{{ route('special.show', $rs->id) }}" type="button" class="btn btn-secondary" data-toggle="tooltip" data-placement="top" title="View Details">Detail</a>
                                @if (Auth::user()->role=='Encoder')
                                <a href="{{ route('special.edit', $rs->id)}}" type="button" class="btn btn-warning" data-toggle="tooltip" data-placement="top" title="Edit">Edit</a>
                                <form action="{{ route('special.destroy', $rs->id) }}" method="POST" type="button" class="btn btn-danger p-0" onsubmit="return confirm('Delete?')"data-toggle="tooltip" data-placement="top" title="Delete">
                                    @csrf
                                    @method('DELETE')
